...
st.set_page_config(
    page_title="Music Matcher AI", page_icon="🎵", layout="wide"
 )
...
def load_artifacts():
    ...
    metadata = pd.read_feather("tracks_metadata.feather")
    return model, scaler, feature_cols, metadata


model, scaler, feature_cols, metadata = load_artifacts()

st.title("🎵 AI Track Finder & Music Matcher")
st.markdown(
    "Set your desired audio characteristics below to find the best matching **Track Name** from the catalog."
)
...
# Convert user categorical inputs into the one-hot columns expected by the model
...
# Assemble input vector in the exact training sequence
input_dict = {
    "energy": energy,
    "mode": mode,
...
if st.button("🚀 Find Matching Track", use_container_width=True):
    ...
    for rank, (idx, dist) in enumerate(zip(indices[0], distances[0]), start=1):
        ...
        with st.container():
            ...
            cols[3].caption(
                f"Genre: {track_info['genre']}\n\nTempo: {track_info['tempo']:.0f} BPM | Energy: {track_info['energy']:.2f}"
            )
            st.divider()